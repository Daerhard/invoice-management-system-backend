package invoice.management.system.controller

import invoice.management.system.api.PurchaseInvoicesApi
import invoice.management.system.api.PurchaseInvoicesApiDelegate
import org.springframework.beans.factory.annotation.Autowired
import org.springframework.stereotype.Controller
import org.springframework.web.bind.annotation.RequestMapping

@Controller
@RequestMapping("\${openapi.invoice-management-system.base-path:}")
class PurchaseInvoicesController(
    @Autowired private val delegate: PurchaseInvoicesApiDelegate
) : PurchaseInvoicesApi {

    override fun getDelegate(): PurchaseInvoicesApiDelegate = delegate
}
